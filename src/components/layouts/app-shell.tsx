...
import { APP_NAME, ROLE_HOME } from "@/lib/constants/app";
...
const navItems: NavItem[] = [
  ...
  { href: "/profile", label: "Profile", icon: UserCircle, roles: ["EMPLOYEE", "MANAGER", "ADMIN"] },
];
  ...
  return (
    <div className={cn("min-h-screen bg-background", highContrast && "contrast-125")} style={{ fontSize: `${fontScale}rem` }}>
      <div className="fixed inset-y-0 left-0 hidden w-72 border-r bg-card/78 backdrop-blur-xl lg:block">
        <Link href={ROLE_HOME[user.role] ?? "/"} className="flex h-16 items-center gap-3 border-b px-5 transition-colors hover:bg-muted/40" aria-label={`${APP_NAME} home`}>
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b bg-background/82 px-4 backdrop-blur-xl sm:px-6">
          <div className="flex flex-1 items-center gap-3">
            ...
            <GlobalSearch items={visibleItems} className="hidden w-72 max-w-md sm:block" />
          </div>
          <div className="flex items-center gap-1 sm:gap-2">
            <Button type="button" variant="outline" size="icon" className="hidden sm:inline-flex" aria-label="Toggle high contrast" onClick={toggleHighContrast}>
              <Accessibility />
            </Button>
            <ThemeToggle />
            <NotificationBell />
